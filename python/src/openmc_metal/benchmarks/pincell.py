...
from ..geometry import build_c5g7_pincell, source_sampler, REFERENCE_KEFF
...
class PincellBenchmark:
    ...
    def run(self, num_particles: int = 100_000,
        ...
        """Run the C5G7 pincell benchmark.

        Returns: dict with simulation results and comparison data.
        """
        print("\n" + "=" * 60)
        print("   C5G7 Pincell K-Eigenvalue Benchmark")
        print("=" * 60)

        # Build geometry
        geometry = build_c5g7_pincell(self.engine)
        ...
        source = source_sampler(num_particles)

        # Run simulation
        result = sim.run(source=source, verbose=verbose)
        result.print_summary(reference=REFERENCE_KEFF)

        # Build benchmark result for comparison
        mean_k, std_k, ci95 = result.k_eff
        delta_pcm = abs(mean_k - REFERENCE_KEFF) * 1e5
        ...
        return {
            ...
            'reference_k_eff': REFERENCE_KEFF,
            'delta_pcm': delta_pcm,
